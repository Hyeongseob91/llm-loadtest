...
| | 다크모드 | ✅ |
| **내보내기** | CSV/Excel 다운로드 | ✅ |
| **AI 분석** | 벤치마크 결과 AI 분석 보고서 | ✅ |
| | Thinking 모델 지원 (Qwen3-VL 등) | ✅ |
| | 전문용어 자동 설명 | ✅ |
| | 분석 보고서 Markdown 다운로드 | ✅ |
| **검증 (Validation)** | 클라이언트-서버 메트릭 교차 검증 | ✅ |
| | Docker 로그 기반 검증 (vLLM) | ✅ |
| | Prometheus 메트릭 검증 | ✅ |
| | HTTP 요청/응답 카운트 비교 | ✅ |
| | Throughput 교차 검증 | ✅ |
| **보안** | API Key 인증 | ✅ |
...
| 클라이언트 메트릭 | 서버 메트릭 | 비교 방식 |
|-----------------|-----------|----------|
| Total Requests | HTTP 200 Count | 정수 비교 |
| Avg Throughput | Engine Generation Throughput | ±허용오차 |
| Error Count | HTTP Error Count | 정수 비교 |

**Docker Log Metrics 카드**:
- **HTTP 200 Count**: 성공한 HTTP 요청 수
- **HTTP Errors**: 에러 응답 수 (4xx, 5xx)
- **Avg Gen Throughput**: vLLM Engine의 평균 토큰 생성 속도
- **Peak KV Cache**: KV 캐시 최대 사용률
...
| **Markdown 다운로드** | 분석 완료 후 보고서를 Markdown 파일로 다운로드 (`analysis_{model}_{date}.md`) |
...
| **GET** | `/api/v1/benchmark/result/{run_id}/analysis` | **AI 분석 보고서 (SSE 스트리밍)** | - |
...
상세 PRD: [docs/prd/validation-loop-prd.md](docs/prd/validation-loop-prd.md)
